...
int SDL_BlitSurface_dh (SDL_Surface* source, SDL_Rect* source_rect, SDL_Surface* dest, SDL_Rect* dest_rect) {
    SDL_LockSurface(source);
    SDL_LockSurface(dest);
    for(int y = 0; y < source_rect->h; y++) {
        char* source_ptr = (char*) source->pixels + (y + source_rect->y) * source->pitch + source_rect->x * source->format->BytesPerPixel;
        char* dest_ptr1 = (char*) dest->pixels + 2 * (y + dest_rect->y) * dest->pitch + dest_rect->x * dest->format->BytesPerPixel;
        char* dest_ptr2 = (char*) dest->pixels + (2 * (y + dest_rect->y) + 1) * dest->pitch + dest_rect->x * dest->format->BytesPerPixel;
        int length = source_rect->w * source->format->BytesPerPixel;
        memcpy(dest_ptr1, source_ptr, length);
        memcpy(dest_ptr2, source_ptr, length);
    }
...
display_movie (void)
{
  ...
  switch (vmode)
    ...
    case 1:
      ...
      if (window_height == 480)
        {
          SDL_UpdateRect (public_surface, 0, 40 * 2,
                          public_surface->w, public_surface->h - 40 * 2);
        }
      else
        SDL_UpdateRect (public_surface, 0, 0,
                        public_surface->w, public_surface->h);
